...
public class CongDien {
    ...
    public void loaiThanhVienCD2(ArrayList<Nhom> dsNhom, LinkedList<AnhTai> dsAnhTaiBiLoai) {
    ...
    public void loaiThanhVienCD3(ArrayList<Nhom> dsNhom, LinkedList<AnhTai> dsAnhTaiBiLoai) {
        TreeSet<Nhom> dsNhomDaSort = new TreeSet<>();
        for (Nhom nhom : dsNhom) {
            dsNhomDaSort.add(nhom);
        }
        ArrayList<Nhom> nhomThapDiem = new ArrayList<>();
        nhomThapDiem.add(dsNhomDaSort.pollFirst());
        nhomThapDiem.add(dsNhomDaSort.pollFirst());
        for(Nhom nhom : nhomThapDiem){
            //sắp xếp danh sách anh tài theo điểm hỏa lực
            for (int i = 0; i < nhom.getDsAnhTai().size(); i++) {
                for (int j = i + 1; j < nhom.getDsAnhTai().size(); j++) {
                    if(nhom.getDsAnhTai().get(i).getDiemHoaLuc()>=nhom.getDsAnhTai().get(j).getDiemHoaLuc()) {
            ...
            // lấy anh tài có điểm hỏa lực thấp nhất của 1 trong 2 nhà thấp điểm nhất
            dsAnhTaiBiLoai.add(nhom.getDsAnhTai().pollFirst());
        }
        //xóa 2 anh tài bị loại khỏi nhóm
        for (AnhTai anhTai : dsAnhTaiBiLoai) {
            for(Nhom nhom : dsNhom){
                if(nhom.getDsAnhTai().contains(anhTai)) {
                    nhom.getDsAnhTai().remove(anhTai);
                }
            }
        }
    }
